...
public class Gui extends JFrame {
    ...
    private void Guiinit(String name,int mode){
        setTitle(name);
        String WindowSizeTmp = Gui.config.LoadConfig(2);
        if(WindowSizeTmp == null || WindowSizeTmp == "") setBounds(10,10,800,600);
        else setBounds(10,10, Integer.valueOf(Gui.config.ConfigSplit(WindowSizeTmp, 0)),Integer.valueOf(Gui.config.ConfigSplit(WindowSizeTmp,1)));
        ...
        addWindowListener(new WindowAdapter() {
            ...
            public void windowClosing(WindowEvent e) {
                ...
                Gui.config.SaveConfig(getSize().width + ":" + getSize().height,true);
                System.out.println(getSize().width);
            }
        });

        File Plugins_File = new File("Plugins");
        if(!Plugins_File.exists()) Plugins_File.mkdirs();
